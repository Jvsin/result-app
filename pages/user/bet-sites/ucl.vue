                          ...
                          <v-row justify="center">
                            <v-col cols="6" class="d-flex flex-column justify-center align-center">
                              <div>
                                ...
                                <div no-wrap class="text-center text-h4">
                                  {{ (pastUserBets.find((bet: IBet)  => bet.matchID === game.id)) ? 
                                    pastUserBets.find((bet: IBet)  => bet.matchID === game.id)?.home + '-'
                                    + pastUserBets.find((bet: IBet)  => bet.matchID === game.id)?.away
                                    : '-' }}
                                </div>
                            </div>
                            </v-col>
                            <v-col cols="6" class="d-flex flex-column justify-center align-center">
                              ...
                              <div v-if="game.status =='FT'" class="text-h4">
                                {{ (pastUserBets.find((bet: IBet)  => bet.matchID === game.id)) ? 
                                  pastUserBets.find((bet: IBet)  => bet.matchID === game.id)?.points : '-' }}
                              </div>
...
const nextGames = computed(() => {
  return betStore.nextGames.filter((game: IMatch) => game.league === "ucl")
})
...
const futureUserBetsData = computed(() => {
  return betStore.futureBetsData?.filter((bet:IMatch) => bet.league === "ucl")
})
...
watch(matchesNumber, async (oldNum:any, newNum:any) => {
  loading.value = true
  await betStore.fetchNextFixturesData(2, matchesNumber.value)
  setBetsToSave()
}, { immediate: true });
...
async function fetchFutureUserBets() {
  if (user.value?.reference) {
    loading.value = true
    await betStore.fetchFutureUserBets(user.value.reference, "ucl")
  }
  loading.value = false
...
function setColor(matchID: Number, status: String) {
  const userBet = pastUserBets.value.find((bet: IBet)  => bet.matchID === matchID);
  if (status == 'FT') {
...
onMounted(async () => {
  ...
  if (user.value?.reference) {
    await betStore.fetchPastUserBets(user.value?.reference, 'ucl')
    await betStore.fetchFutureUserBets(user.value?.reference, 'ucl')
