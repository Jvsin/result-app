  ...
  <v-dialog :model-value="isShowRef" max-width="700" scrollable @update:model-value="close" >
    <v-card>
      ...
      <v-form ref="form" v-model="valid" class="px-5" @submit.prevent="editProfile">
        <v-text-field :label="$t('user.betLeaguesSites.editDialog.leagueName')" v-model="leagueName" variant="outlined" :rules="[requiredRule(), lengthRuleShort(), lengthRule()]"></v-text-field>
        <v-text-field :label="$t('user.betLeaguesSites.editDialog.description')" v-model="description" variant="outlined" :rules="[requiredRule(), lengthRule()]"></v-text-field>
      </v-form>
      ...
      <v-card-actions>
        ...
        <v-btn color="primary" @click="editProfile">{{$t('user.betLeaguesSites.editDialog.save')}}</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
